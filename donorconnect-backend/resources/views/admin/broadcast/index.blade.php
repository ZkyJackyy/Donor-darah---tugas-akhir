...
<div class="space-y-6">
    ...
    <div>
        <h1 class="font-bold text-2xl text-gray-900 tracking-tight">Broadcast WhatsApp</h1>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="bg-white rounded-2xl border border-gray-100 shadow-card p-5 hover:shadow-md transition-shadow">
            ...
            <div class="text-3xl font-bold text-gray-900 tracking-tight">{{ $totalSent }}</div>
        </div>

        <div class="bg-white rounded-2xl border border-gray-100 shadow-card p-5 hover:shadow-md transition-shadow">
            <div class="flex items-center gap-3 mb-3">
                <div class="w-10 h-10 rounded-xl bg-brand-50 flex items-center justify-center">
                    <svg class="w-5 h-5 text-brand-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                        <circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/>
                    </svg>
                </div>
                <span class="text-xs font-semibold text-gray-400 uppercase tracking-wide">Gagal</span>
            </div>
            <div class="text-3xl font-bold text-gray-900 tracking-tight">{{ $totalFailed }}</div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-card p-5 hover:shadow-md transition-shadow">
            ...
            <div class="text-3xl font-bold text-gray-900 tracking-tight">{{ $totalPending }}</div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-card overflow-hidden">
        <div class="px-6 py-5 border-b border-gray-100">
            <h2 class="text-sm font-bold text-gray-900">Riwayat Broadcast</h2>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left">
                <thead>
                    <tr>
                        <th class="text-xs font-semibold text-gray-400 uppercase tracking-wider px-6 py-4 border-b border-gray-100 bg-gray-50/50">Tanggal</th>
                        <th class="text-xs font-semibold text-gray-400 uppercase tracking-wider px-6 py-4 border-b border-gray-100 bg-gray-50/50">Nomor HP</th>
                        <th class="text-xs font-semibold text-gray-400 uppercase tracking-wider px-6 py-4 border-b border-gray-100 bg-gray-50/50">Pesan</th>
                        <th class="text-xs font-semibold text-gray-400 uppercase tracking-wider px-6 py-4 border-b border-gray-100 bg-gray-50/50">Status</th>
                    </tr>
                </thead>
        ...
        <div class="px-6 py-4 border-t border-gray-100 flex items-center justify-between bg-gray-50/30">
            ...
            <div class="flex gap-2">
                @if($broadcasts->onFirstPage())
                    <span class="border border-gray-200 rounded-lg px-3.5 py-1.5 text-sm text-gray-300 cursor-not-allowed">Sebelumnya</span>
                @else
                    <a href="{{ $broadcasts->previousPageUrl() }}" class="border border-gray-200 rounded-lg px-3.5 py-1.5 text-sm text-gray-600 hover:bg-gray-50 transition-colors font-medium">Sebelumnya</a>
                @endif
                @if($broadcasts->hasMorePages())
                    <a href="{{ $broadcasts->nextPageUrl() }}" class="border border-gray-200 rounded-lg px-3.5 py-1.5 text-sm text-gray-600 hover:bg-gray-50 transition-colors font-medium">Selanjutnya</a>
                @else
                    <span class="border border-gray-200 rounded-lg px-3.5 py-1.5 text-sm text-gray-300 cursor-not-allowed">Selanjutnya</span>
                @endif
            </div>
        </div>
        @endif
