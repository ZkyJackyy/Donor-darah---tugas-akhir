...
document.addEventListener('alpine:init', () => {
    Alpine.data('donorMap', () => ({
        ...
        init() {
            this.$nextTick(() => {
                this.map = L.map('map').setView([{{ config('donorconnect.default_lat') }}, {{ config('donorconnect.default_lng') }}], 13);
                ...
                L.marker([{{ config('donorconnect.default_lat') }}, {{ config('donorconnect.default_lng') }}], { icon: pmiIcon })
                    .addTo(this.map)
                    .bindPopup('<div style="font-family:Plus Jakarta Sans,sans-serif;padding:4px 0"><b style="font-size:13px">{{ config('donorconnect.default_hospital_name') }}</b><br><span style="font-size:11px;color:#64748b">{{ config('donorconnect.default_hospital_address') }}</span></div>');

                this.circle = L.circle([{{ config('donorconnect.default_lat') }}, {{ config('donorconnect.default_lng') }}], {
                    radius: 5000,
                    color: '#3B82F6',
                    fillColor: '#3B82F6',
                    fillOpacity: 0.04,
        ...
        renderMarkers() {
            ...
            const cooldownIcon = L.divIcon({
                html: '<div style="background:#FBBF24;width:12px;height:12px;border-radius:50%;border:2.5px solid white;box-shadow:0 2px 6px rgba(0,0,0,0.2)"></div>',
                iconSize: [12, 12],
                className: ''
            });

            this.donors.forEach(d => {
                const marker = L.marker([d.latitude, d.longitude], { icon: d.is_available ? donorIcon : cooldownIcon })
                    .addTo(this.map)
                    .bindPopup(`
                        <div style="font-family:'Plus Jakarta Sans',sans-serif;padding:4px 0">
                            ...
                            <span style="font-size:11px;color:#94A3B8">${d.last_donor_date ? 'Terakhir donor: ' + d.last_donor_date : 'Belum pernah donor'}</span><br>
                            <span style="font-size:11px;font-weight:bold;color:${d.is_available ? '#3B82F6' : '#B45309'}">${d.is_available ? 'Siap Donor' : 'Masa Cooldown'}</span>
                        </div>
                    `);
