...
<head>
    ...
    <style>
        ...
        body {
            background-color: #faf9f7; /* medical.soft */
            color: #1f2933;
...
<body class="flex min-h-screen bg-medical-soft" x-data="{ sidebarOpen: false, userDropdownOpen: false }">
    ...
    <!-- Sidebar -->
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-50 w-72 bg-white border-r border-gray-200 transform transition-transform duration-300 lg:translate-x-0 lg:static lg:flex lg:flex-col">
        <!-- Logo -->
        ...
        <nav class="flex-1 overflow-y-auto px-4 py-6 space-y-1">
            <p class="px-4 text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3 mt-2">Utama</p>
            
            <a href="{{ route('admin.dashboard') }}" class="sidebar-item flex items-center gap-3 px-4 py-2.5 rounded-md text-sm font-medium text-gray-600 {{ request()->routeIs('admin.dashboard') ? 'active' : '' }}">
            ...
            <p class="px-4 text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3 mt-6">Manajemen Darah</p>

            <a href="{{ route('admin.donors') }}" class="sidebar-item flex items-center gap-3 px-4 py-2.5 rounded-md text-sm font-medium text-gray-600 {{ request()->routeIs('admin.donors') ? 'active' : '' }}">
            ...
            <a href="{{ route('admin.blood-requests.index') }}" class="sidebar-item flex items-center justify-between px-4 py-2.5 rounded-md text-sm font-medium text-gray-600 {{ request()->routeIs('admin.blood-requests.*') && !request()->routeIs('admin.blood-requests.pending') ? 'active' : '' }}">
                <div class="flex items-center gap-3">
            ...
            <a href="{{ route('admin.verify.index') }}" class="sidebar-item flex items-center gap-3 px-4 py-2.5 rounded-md text-sm font-medium text-gray-600 {{ request()->routeIs('admin.verify.*') ? 'active' : '' }}">
                <svg class="w-5 h-5 {{ request()->routeIs('admin.verify.*') ? 'text-brand-600' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                Verifikasi Kode
            </a>

            <a href="{{ route('admin.alerts.index') }}" class="sidebar-item flex items-center justify-between px-4 py-2.5 rounded-md text-sm font-medium text-gray-600 {{ request()->routeIs('admin.alerts.*') ? 'active' : '' }}">
                <div class="flex items-center gap-3">
            ...
            <p class="px-4 text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3 mt-6">Sistem</p>

            <a href="{{ route('admin.broadcast.index') }}" class="sidebar-item flex items-center gap-3 px-4 py-2.5 rounded-md text-sm font-medium text-gray-600 {{ request()->routeIs('admin.broadcast.*') ? 'active' : '' }}">
                <svg class="w-5 h-5 {{ request()->routeIs('admin.broadcast.*') ? 'text-brand-600' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.111 16.404a5.5 5.5 0 017.778 0M12 20h.01m-7.08-7.071c3.904-3.905 10.236-3.905 14.141 0M1.394 9.393c5.857-5.857 15.355-5.857 21.213 0"></path></svg>
                Riwayat Broadcast
            </a>

            <a href="{{ route('admin.reports.index') }}" class="sidebar-item flex items-center gap-3 px-4 py-2.5 rounded-md text-sm font-medium text-gray-600 {{ request()->routeIs('admin.reports.*') ? 'active' : '' }}">
                <svg class="w-5 h-5 {{ request()->routeIs('admin.reports.*') ? 'text-brand-600' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                Laporan & Statistik
            </a>

            <a href="{{ route('admin.settings.index') }}" class="sidebar-item flex items-center gap-3 px-4 py-2.5 rounded-md text-sm font-medium text-gray-600 {{ request()->routeIs('admin.settings.*') ? 'active' : '' }}">
        ...
        <div class="p-4 border-t border-gray-100">
            <form action="{{ route('admin.logout') }}" method="POST">
                @csrf
                <button type="submit" class="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-white border border-gray-200 text-gray-700 rounded-md text-sm font-semibold hover:bg-gray-50 hover:text-brand-600 hover:border-brand-200 transition-colors">
    ...
    <main class="flex-1 flex flex-col min-h-screen overflow-hidden w-full">
        <!-- Topbar -->
        <header class="h-20 bg-white border-b border-gray-200 flex items-center justify-between px-6 lg:px-10 sticky top-0 z-30">
            <div class="flex items-center gap-4">
                <button @click="sidebarOpen = true" class="lg:hidden p-2 rounded-md bg-white border border-gray-200 text-gray-500 hover:text-brand-600">
                ...
                <div class="relative" x-data="{ open: false }">
                    <button @click="open = !open" @click.away="open = false" class="flex items-center gap-3 focus:outline-none">
                        <div class="hidden sm:block text-right">
                            <p class="text-sm font-semibold text-gray-800 leading-tight">{{ auth()->user()->name ?? 'Admin PMI' }}</p>
                            <p class="text-[10px] text-brand-600 font-semibold uppercase tracking-wide">Super Admin</p>
                        </div>
                        <div class="relative">
                            <div class="w-9 h-9 rounded-full bg-brand-50 border border-brand-200 flex items-center justify-center text-brand-600 font-semibold">
                    ...
                    <!-- Dropdown Menu -->
                    <div x-show="open" x-cloak x-transition.opacity.duration.200ms class="absolute right-0 mt-3 w-48 bg-white rounded-md shadow-lg border border-gray-200 overflow-hidden py-1 z-50">
        ...
        <div class="flex-1 overflow-y-auto p-6 lg:p-10">
            @if(session('success'))
                <div class="mb-6 bg-emerald-50 border border-emerald-200 text-emerald-800 px-4 py-3 rounded-md flex items-start gap-3" x-data="{ show: true }" x-show="show">
                    <svg class="w-4 h-4 text-emerald-600 mt-1 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path></svg>
                    <div class="flex-1">
                        <h4 class="text-sm font-semibold text-emerald-900">Berhasil</h4>
            ...
            @if(session('error'))
                <div class="mb-6 bg-red-50 border border-red-200 text-red-800 px-4 py-3 rounded-md flex items-start gap-3" x-data="{ show: true }" x-show="show">
                    <svg class="w-4 h-4 text-red-600 mt-1 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path></svg>
                    <div class="flex-1">
